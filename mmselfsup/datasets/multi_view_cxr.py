...
class MultiViewDatasetMIMIC(BaseDataset):
...
@DATASETS.register_module()
class MultiViewDatasetCXP(BaseDataset):
    """The dataset outputs multiple views of an image.
...
@DATASETS.register_module()
class MultiViewDatasetNIH(BaseDataset):
    """The dataset outputs multiple views of an image.
...
@DATASETS.register_module()
class MultiViewDatasetPDC(BaseDataset):
    """The dataset outputs multiple views of an image.
    ...
    def __init__(self, data_source, num_views, pipelines, prefetch=False):
        assert len(num_views) == len(pipelines)
        # self.data_source = build_datasource(data_source)
        self.data_source = np.load('/raid/cl522/XSCAN/all_data/PDC.npy', 'r')
        self.pipelines = []
        for pipe in pipelines:
            pipeline = Compose([build_from_cfg(p, PIPELINES) for p in pipe])
            self.pipelines.append(pipeline)
